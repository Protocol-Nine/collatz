...
\begin{table} [ht]
    \begin{center}
        \begin{tabular}{|l|c|c|c|r|}
            \hline
                                                        & \textbf{j} & $\textbf{k}_o$ & $\textbf{k}_z$ & \textbf{binary} \\
            \hline
...
Whenever transitioning to an even value such that $(\text{even}\mod 3 = 1)$, then the previous value could have been either:
...
\subsection{Randomization Phase}
...
\begin{itemize}
    \item To overcome a contrived Seed, one way hashing smoothes out any regularities.
